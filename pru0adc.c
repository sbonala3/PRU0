...
int main(void) {
  ...
  int i = 0;
  //origional for loop wanted 24 bits at a time but only took 16?
  while (1) {
	 // spiCommand is the MOSI preamble; must be reset for each sample.
     spiCommand = 0x80000000;
     ...
     spiCommand = spiCommand << 1;   // Shift left one bit (for next cycle).
      ...
      __delay_cycles(78); //  Delay to allow settling.
